...
- (void)playVideo {
    [yVideoPlayerView clean];
    YVideoPlayerView * y2 = [[YVideoPlayerView alloc]initWithFrame:CGRectMake(0, 200, _videoView.frame.size.width, _videoView.frame.size.height) VideoName:names.firstObject Path:paths[1] OnViewController:self];
    NSLog(@"y2=%@",y2);
}

- (void)viewDidAppear:(BOOL)animated {
       //加载第一个视频
    yVideoPlayerView = [[YVideoPlayerView alloc]initWithFrame:CGRectMake(0, 20, _videoView.frame.size.width, _videoView.frame.size.height) VideoName:names.firstObject Path:paths.firstObject OnViewController:self];
    UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(playVideo)];
    _videoView2.userInteractionEnabled = YES;
    [_videoView2 addGestureRecognizer:tap];
